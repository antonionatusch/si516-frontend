...
const ClinicHistoryCreate = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const pickupObj = formData.pickup
                ? { description: formData.pickup.trim() }
                : {}
                ...
                <CForm onSubmit={handleSubmit}>
                    <CRow className="mb-3">
                        <CCol md={6}>
                            ...
                            <CFormInput name="patientId" value={formData.patientId} onChange={handleChange} required />
                        </CCol>
                        <CCol md={6}>
                            <CFormLabel>Médico (ID)</CFormLabel>
                            <CFormInput name="doctorId" value={formData.doctorId} onChange={handleChange} required />
                        </CCol>
                    </CRow>

                    <CFormLabel>Motivo de la visita</CFormLabel>
                    <CFormInput name="visitReason" value={formData.visitReason} onChange={handleChange} required className="mb-3" />

                    <CFormLabel>Diagnóstico</CFormLabel>
                    <CFormInput name="diagnosis" value={formData.diagnosis} onChange={handleChange} required className="mb-3" />

                    <CFormLabel>Síntomas (separados por coma)</CFormLabel>
                    <CFormTextarea name="symptoms" value={formData.symptoms} onChange={handleChange} rows={2} className="mb-3" />

                    <CFormLabel>Tratamiento (JSON)</CFormLabel>
                    <CFormTextarea name="treatment" value={formData.treatment} onChange={handleChange} rows={3} className="mb-3" />

                    <CFormLabel>Recogida (pickup) (JSON)</CFormLabel>
                    <CFormTextarea name="pickup" value={formData.pickup} onChange={handleChange} rows={3} className="mb-3" />
